define([
    ...
    var viewModel = function(params) {
        ...
        this.applyBuffer = function(val){
          var self = this;
          var buffer = ko.unwrap(val)
          if (buffer && buffer > 0) {
              this.draw.getAll().features.forEach(function(feature){
                if (feature.geometry.coordinates.length > 0) {
                  self.draw.add(self.bufferFeature(feature));
                }
              })
              this.saveGeometries();
              this.drawingAdded(true);
          };
        }
        ...
        this.setupMap = function(map) {
            ...
            this.saveGeometries = function() {
                ...
                if (self.value.features !== undefined) {
                    ...
                    var validFeatures = currentDrawing.features.filter(function(feature) {
                        if (feature.geometry.coordinates.length > 0) {
                          return feature;
                        }
                    });
                    currentDrawing.features = validFeatures;
